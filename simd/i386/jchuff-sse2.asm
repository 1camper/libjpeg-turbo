...
%ifdef WIN32
%define ALIGNED_ESP 1
%else
%define ALIGNED_ESP 0
%endif
...
%macro STORE_T 2-3 0
    punpcklwd   xmm6, %1
    punpckhwd   xmm7, %1
    movaps      XMMWORD [t + %2 * SIZEOF_T + %3], xmm6
    movaps      XMMWORD [t + %2 * SIZEOF_T + %3 + 16], xmm7
%endmacro
%else
%assign SIZEOF_T 2
%assign OFFSET_T 0
%assign SHIFT_T  1
%macro STORE_T 2-3 0
    movaps      XMMWORD [t + %2 * SIZEOF_T + %3], %1
%endmacro
%endif
...
%macro EMIT_QWORD 8 ; temp, temph, tempb, insn1, insn2, insn3, insn4, insn5
%define %%temp     %1
%define %%temph    %2
%define %%tempb    %3
%define %%insn1    %4
...
%endmacro
    ...
    lea         t, [t - SIZEOF_T]                                                      ; t = &t[-1]
    pxor        xmm0, xmm0                               ; w0[i] = 0
    pcmpgtw     xmm2, xmm1                               ; w2[i] = -(w2[i] < w1[i])
    paddw       xmm1, xmm2                               ; w1[i] += w2[i]
    STORE_T     xmm1, 40, SIZEOF_T                       ; t[40+i] = w1[i]
...
%define nbitsb dl
%undef block
    movzx       nbits, byte [NBITS(code_temp)]                                         ; nbits = JPEG_NBITS(code_temp);
...
%define sizeb cl
    ...
    STORE_T     xmm5, 32, SIZEOF_T                       ; t[32+i] = w5[i]
...
; if size == 32 index remains unchanged, correct in BRLOOP
    ...
    lea         t, [t + size * SIZEOF_T]                                                       ;    t += size;
    cmp         size, 16                                                                       ;    if (size > 16)
    jg          .BRLOOP1                                                                       ;        goto .BRLOOP1;
.ERLOOP1:                                                                                      ; .ERLOOP1:
    movsx       nbits, word [t + OFFSET_T]                                                     ;     nbits = *t;
%ifdef PIC
...
.BEGIN:                                                                                        ; .BEGIN:
    ...
    movd        code_bits, nbits                                                               ;     code_bits = nbits;
    psllq       put_buffer, code_bits                                                          ;     put_buffer <<= code_bits;
    lea         size, [size + 1]                                                               ;     ++size;
    por         put_buffer, code                                                               ;     put_buffer |= code;
    jnz         .BLOOP1                                                                        ; } while (index != 0);
; round 2
; t points to the last used word, possibly below t_ if previous index had 32 zero bits
.ELOOP1:
    pmovmskb    size, xmm4                                                                     ; size = index_mask[48..63];
    pmovmskb    index, xmm5                                                                    ; index = index_mask[32..47];
    shl         size, 16                                                                       ; size <<= 16;
    or          index, size                                                                    ; index |= size;
    not         index                                                                          ; index = ~index;
    lea         nbits, [t + (1 + DCTSIZE2) * SIZEOF_T]                                         ; nbits = t + 1 + 64
    and         nbits, -DCTSIZE2 * SIZEOF_T                                                    ; nbits &= -{128,256} // now points at &t_[64]
    sub         nbits, t                                                                       ; nbits -= t;
    shr         nbits, SHIFT_T ; #leading-zero-bits_old_index + 33                             ; nbits >>= {1,2};
    tzcnt       size, index                                                                    ; size = #trailing 0-bits in index
...
.ERLOOP2:                                                                                      ; .ERLOOP2:
    ...
    por         code, mmtemp                                                                   ;     code |= mmtemp;
    xor         size, size ; kill tzcnt input dependency
    sub         free_bits, nbits                                                               ;     if ((free_bits -= nbits) <= 0)
...
.ELOOP2:                                                                                       ; .ELOOP2:
    mov         nbits, t                                                                       ; nbits = t;
    lea         t, [t + SIZEOF_T]                                                              ; t = &t[1];
    and         nbits, DCTSIZE2 * SIZEOF_T - 1                                                 ; nbits &= {127,255};
    and         t, -DCTSIZE2 * SIZEOF_T                                                        ; t &= -{128,256}; // &t_[0]
    cmp         nbits, (DCTSIZE2 - 2) * SIZEOF_T                                               ; if (nbits == 62 * {2,4})
    je          .EFN                                                                           ;     goto .EFN;
